...
import { buildPostCompactionReminder as corePostCompactionReminder } from '../compact-instructions.js';
 ...
 * auto-compacts the active session. A summary can lose routing instructions;
 * re-state the core delivery contract with the current destinations on the
 * next prompt. Isolated tasks use core's tool-delivery wording.
 *
 * The wording is shared with the Claude PreCompact path
 * (`buildPostCompactionReminder` in compact-instructions.ts) so the two cannot drift.
 *
...
export function buildPostCompactionReminder(
  ...
  return corePostCompactionReminder(names, taskId);
}
...
export class OpenCodeProvider implements AgentProvider {
  ...
  query(input: QueryInput): AgentQuery {
    ...
    // Core owns ordinary envelope delivery and isolated task delivery wording.
    const compaction = createCompactionReminder(() => buildPostCompactionReminder(undefined, getTaskSeriesId()));
          ...
          const timeoutCheck = setInterval(
            () => {
              ...
              if (!streamSilent && now - lastEventAt > STREAM_SILENCE_MS) {
                ...
                // down. Stream release prevents reconnection, and terminating
                // the owned server wakes the parked read. Both are needed.
                // This tier has done its job: stop ticking.
                clearInterval(timeoutCheck);
                log(
                  `OpenCode event stream silent for ${STREAM_SILENCE_MS}ms — dropping runtime, session ${turnSessionId}`,
                );
                streamSilent = true;
                self.activeSessionId = undefined;

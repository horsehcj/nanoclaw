...
describe('verified OpenCode turn completion', () => {
  ...
  it('surfaces an error stored on a successful HTTP response', async () => {
    const f = fixture();
    const result = collect(f.run());
    await prompted(f);
    f.complete(
    ...
    await expect(result).rejects.toThrow('backend rejected request');
    expect(f.abort).toHaveBeenCalledTimes(1);
  });

  it('aborts native execution at the retry limit before a second turn can start', async () => {
    const f = fixture();
    const first = collect(f.run());
    await prompted(f);
    const second = collect(f.run());
    f.emit({ type: 'session.status', properties: { sessionID: 'ses_test', status: { type: 'retry', attempt: 3 } } });
    await expect(first).rejects.toThrow('retry limit');
    expect(f.abort).toHaveBeenCalledTimes(1);
    await prompted(f, 2);
    f.complete();

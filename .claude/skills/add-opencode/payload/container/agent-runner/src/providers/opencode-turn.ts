...
export async function* executeOpenCodeTurn(options: {
  ...
  prepare?(): void;
  discard(): void;
}): AsyncGenerator<{ type: 'activity' }, string | null> {
  ...
  const unlisten = pump.listen((event) => {
    ...
    if (event.type === 'session.status' && record.sessionID === sessionId) {
      const status = properties.status as { type?: string; attempt?: number } | undefined;
      if (status?.type === 'retry' && (status.attempt ?? 0) >= 3) {
        return fail(new Error(`OpenCode retry limit (${status!.attempt}); aborted`));
      }
    }
  ...
  try {
    ...
    const result = await boundedOpenCodeCall(
      (signal) =>
        reconcileOpenCodeTurn(
          client,
          sessionId,
          messageId,
          response!.data!,
...
export async function reconcileOpenCodeTurn(
  ...
  if (result.info.error) throw openCodeError(result.info.error);
  let messages: OpenCodeMessage[] = [];
  let marker: OpenCodeMessage | undefined;
  for (let limit = 100; limit <= 6400; limit *= 2) {
    const response = await client.messages({ path: { id: sessionId }, query: { limit }, signal });
    if (response.error) throw openCodeError(response.error);

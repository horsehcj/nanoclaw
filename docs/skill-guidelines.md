...
| `nanoclaw-provider-host-harness-module` | Optional relative module path inside the skill | Pre-install host maintenance: detect/install CLI, native configuration, failure/help handoff, operational skills. Absence means legacy/unclaimed support. See [the host maintenance contract](provider-host-maintenance.md). |

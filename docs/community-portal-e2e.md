...
Use a fresh exe.dev VM with Docker, Node 22 or newer, pnpm 10.34.5 and native build
...
The portal client is ordinary TypeScript under `src/community-portal/` with no
dependencies beyond Node built-ins; its wire contract with the hosted portal is
pinned by `src/community-portal/wire-vectors.json`. The host-side runtime is the
`community-portal` module under `src/modules/`.
...
checkout changes with an owner-stamped lock file (`data/setup-mutation.lock`); a
crashed owner is recognised by its process birth time and reclaimed.
